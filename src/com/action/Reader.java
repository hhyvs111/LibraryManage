...
public class Reader extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        }else if("readerLogin".equals(action)){
    ...
    public ActionForward readerLogin(ActionMapping mapping, ActionForm form,
		...
		System.out.println(readerForm.getBarcode());
		System.out.println(readerForm.getPaperNO());
		...
		if (ret == 1) {
			...
			session.setAttribute("barcode", readerForm.getBarcode());  //将读者编号写入session
			return mapping.findForward("readerLoginok");
		} else {
			request.setAttribute("error", "您输入的读者名称或密码错误！");
			return mapping.findForward("error");
		}
	}
    /***********************添加读者信息**************************/
